...
export const InviteMemberDialog: React.FC<InviteMemberDialogProps> = ({
  ...
  const handleSendEmailInvite = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const res: any = await fairtabApi.invitations.createEmail({
        groupId,
        email,
        role: emailRole,
      });
        ...
        {generatedLink ? (
          <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-2 p-4 bg-success/5 border border-success/20 rounded-xl">
              <span className="text-sm font-semibold text-text-primary">Invitation Link Created!</span>
              <p className="text-xs text-text-secondary leading-relaxed">
                {activeTab === "email"
                  ? "Share this link with your invited member. They can join immediately."
                  : "Share this link with anyone. They must request to join, subject to admin approval."}
              </p>
            </div>
